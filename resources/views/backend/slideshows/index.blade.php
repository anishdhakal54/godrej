            ...
            <div class="col-xs-12">
                ...
                <div class="box">
                    <div class="box-header">
                        ...
                        <a href="{{ route('dashboard.slideshows.create') }}" class="btn btn-sm btn-danger pull-right">Add
                            New</a>
                    </div>
                    <!-- /.box-header -->
                    <div class="box-body">
                        <table id="example1" class="table table-bordered table-striped datatable">
                            ...
                            @foreach($slideshows as $slideshow)
                                <tr>
                                    ...
                                    <td><img src="{{asset('storage/'.$slideshow->featured_image) }}"
                                             style="width:50px;"></td>
                                    <td>{{$slideshow->name}}</td>
                                    <td>{{$slideshow->link}}</td>
                                    <td>{{$slideshow->created_at}}</td>
                                    <td><a class="text-primary" href="{{ route('dashboard.slideshows.edit', $slideshow->id) }}">Edit </a>|| <a>Delete</a></td>
                                    {{--                                <td>{{$slideshow->}}</td>--}}
                                    {{--                                <td>{{$slideshow->}}</td>--}}
                                </tr>
                            @endforeach
